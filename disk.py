...
def doDisk(file, disk, disk_size, force_format=False, root_partition_size=None):
    if(force_format):
        parted_cmd = 'parted -s '+disk+' mklabel gpt';
        print(parted_cmd);
        process = subprocess.Popen(parted_cmd, shell=True, stdout=subprocess.PIPE,
...
def get_parted_disk_list():
    process = subprocess.Popen('parted -l -m', shell=True, stdout=subprocess.PIPE);
    stdout_str = process.communicate()[0];
    list = stdout_str.split('\n');
    #First initialize uninitialized disks - create gpt if needed
    for line in list:
        if(line.find('Error') != -1 and line.find('unrecognised disk label') != -1): 
            ...
            if(len(tokens) > 2):
                ...
                subprocess.call('parted -s '+disk+' mktable gpt', shell=True);
